...
fn get_file(req_write: bool) -> io::Result<File> {
    let result = File::options().read(true).write(req_write).truncate(req_write).open(FILE_PATH);
    ...
    let file = match result {
        ...
        Err(err) => {
            match err.kind() {
                ErrorKind::NotFound => {
                    OpenOptions::new().read(true).write(true).create_new(true).open(FILE_PATH)?
                }
                _ => {
                    return Err(err)
                }
